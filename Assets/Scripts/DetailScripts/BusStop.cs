...
class BusStop : DetailBase
{
    ...
    public float minBusDistance = 15f;

    private bool spawnLoopStarted = false;
    ...
    private void HandleAllModulesLoaded()
    {
        ...
        if (!spawnLoopStarted)
        {
            ...
            StartCoroutine(BusSpawnLoop());
        }
    }

    private IEnumerator BusSpawnLoop()
    {
        while (true)
        {
    ...
    private void TrySpawnBus()
    {
        ...
        if (player != null)
        {
            float distToPlayer = Vector3.Distance(player.position, stoppoint.transform.position);
            if (distToPlayer < minPlayerDistance)
                return;
        }

        // Проверка наличия других автобусов в радиусе
        Collider[] hits = Physics.OverlapSphere(stoppoint.transform.position, minBusDistance);
        foreach (var h in hits)
        {
            if (h.CompareTag("Bus"))   // Важно: автобус должен иметь тег Bus
            {
                return;               // рядом уже есть автобус → отменяем спавн
            }
        }
    ...
    private void FindStopPoint()
    {
        ...
        if(!transportElementsMaker || transportElementsMaker == null)
        {
            transportElementsMaker = FindAnyObjectByType<TransportElementsMaker>();

            if (!transportElementsMaker || transportElementsMaker == null)
            {
                Debug.LogError("Can't get transportElementsMaker");
                return;
            }
        }
    ...
    {
        if (!routeMaker || routeMaker == null)
        {
            routeMaker = FindAnyObjectByType<RouteMaker>();

            if (!routeMaker || routeMaker == null)
            {
                Debug.LogError("Can't get routeMaker");
                return;
            }
        }
