...
public class AdvancedAIPedestrianController : MonoBehaviour
{
    ...
    // �������� � ���������� ������
    [Header("Smart Crossing")]
    public float timeToCross = 5.0f; // ������ ������� �� �������
    ...
    public float interactionProbability = 0.7f; // ���������� �� InteractiveObject
    ...
    void Start()
    {
        ...
        if (pedestrianRigidbody == null)
        {
            ...
            pedestrianRigidbody.linearDamping = 2f;
            pedestrianRigidbody.angularDamping = 2f;
    ...
    void Update()
    {
        ...
        if (currentState == PedestrianState.FollowingPath &&
            ...
            Random.value < interactionProbability * Time.deltaTime) // ���������� ����� �����������
        {
    ...
    // ��������/��������� ����� CheckForVehicles ��� ������� ������� �� ������������ (TTC)
    bool IsSafeToCross()
    {
        Collider[] vehicles = Physics.OverlapSphere(transform.position, crossingCheckDistance, vehicleMask);

        foreach (var vCol in vehicles)
        {
            AICarController car = vCol.GetComponent<AICarController>();
            if (car == null) continue;

            // ������ � ������
            Vector3 toCar = vCol.transform.position - transform.position;
            float dist = toCar.magnitude;

            // ���� ������ ��� �������� ��� (�����) - ����������
            if (Vector3.Dot(transform.forward, toCar.normalized) < -0.5f) continue;

            // ������ �������� ������ (���� ���� Rigidbody, ����� ����� �� �������)
            Rigidbody carRb = vCol.GetComponent<Rigidbody>();
            float carSpeed = carRb ? carRb.linearVelocity.magnitude : car.maxSpeed; // fallback

            if (carSpeed < 0.1f) continue; // ������ �����

            float timeToImpact = dist / carSpeed;

            // ���� ������ ������� �������, ��� �� �������� + ����� ������������
            if (timeToImpact < timeToCross + 2.0f)
            {
                return false; // ������!
            }
        }
